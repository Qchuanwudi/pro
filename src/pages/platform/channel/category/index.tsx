...
import { queryAppChannelTypeList, addAppChannelType, updateAppChannelType, deleteAppChannelType } from './service';
...
const handleAdd = async fields => {
  ...
  const response = await addAppChannelType(fields);
  if (response.code === 0) {
    hide();
...
const handleUpdate = async fields => {
  ...
  const response = await updateAppChannelType(fields);
  if (response.code === 0) {
    hide();
...
const handleRemove = async selectedRows => {
  ...
  const response = await deleteAppChannelType(data);
  if (response.code === 0) {
    ...
    <GridContent>
      <ProTable<TableListItem>
        ...
        request={async (params = {}) => {
          ...
          const response = await queryAppChannelTypeList(pagination);
          return {
