...
//账号禁用
async (channelId: string, status: string) => {
  const result = await queryAppChannelById(channelId);
...
export interface ChannelFormValueType extends Partial<TableListItem> {
  target?: string;
  template?: string;
  type?: string;
  time?: string;
...
export interface ChannelFormProps extends FormComponentProps {
...
class ChannelForm extends Component<BasicProps, BasicState> {
  componentDidMount() {
...
export default connect(
  ({
    ...
    profileAndbasic,
    loading: loading.effects['profileAndbasic/queryBasicProfile'],
  }),
)(ChannelForm);
